...
```{r, eval=FALSE}
set_contrasts(diamonds, clarity ~ contr.poly - 4:7)
```
...
We can also clean up the scheme names if we want to remove the "contr." and 
"_code" parts.
...
contrast_info <- glimpse_contrasts(tstdf, 
                                   ...
                                   return.list = TRUE,
                                   clean.schemes = TRUE)
contrast_info$glimpse
```

Note that `contr.poly` will be changed to `orth_polynomial` when you do this
to denote that you're using *orthogonal* polynomials and not raw polynomials,
since just `poly` could be confusing.
